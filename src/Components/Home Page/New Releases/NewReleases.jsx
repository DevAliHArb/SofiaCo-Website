...
const NewReleases = () => {
  ...
  const fetchArticles = async () => {
    ...
    try {
      ...
      const url = `${import.meta.env.VITE_TESTING_API}/articles?ecom_type=sofiaco${articleFamilleIdParam}${articlefamilleSubIdParam}&page=1&user_id=${user?.id ? user.id : null}`;
  ...
  const hasPriceRange = (product) =>
    Number(product?._prix_public_ttc_max || 0) >
    Number(product?._prix_public_ttc || 0);
          ...
          <div className={classes.header_low}>
            ...
            <div className={classes.cat_btns_wrapper}>
              ...
              >
                <button
                  className={
                    selectedsubCategoryId === null || selectedsubCategoryId === "null"
                      ? classes.selectedCategory
                      : classes.cat_btns_button
                  }
                  onClick={() => {
                ...
                {authCtx.articleFamille?.map((item) => (
                  <button
                    key={item.id}
                    className={
                      selectedsubCategoryId === Number(item?.id)
                        ? classes.selectedCategory
                        : ""
                    }
                    onClick={() => handleCategoryClick(item?.id)}
                    style={{
                      fontWeight: selectedsubCategoryId === Number(item?.id) && "600",
                      textDecoration:
                        selectedsubCategoryId === Number(item?.id) && "underline",
                    }}
                  >
                    {item?.type_nom}
                  </button>
              ...
              >
                <IoMdArrowBack
                  className={classes.cat_arrow_icon}
                  style={{ transform: "rotate(180deg)" }}
                />
              </button>
            </div>
