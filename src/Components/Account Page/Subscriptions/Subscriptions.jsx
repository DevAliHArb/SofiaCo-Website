...
const Subscriptions = () => {
  ...
  const fetchCollaborators = async () => {
    ...
    if (collaboratorIds.length === 0) {
      ...
      const url = `${import.meta.env.VITE_TESTING_API}/collaborators?${queryParams}`;
  ...
  const fetchCollections = async () => {
    ...
    if (collectionIds.length === 0) {
      ...
      const url = `${import.meta.env.VITE_TESTING_API}/collections?${queryParams}`;
  ...
  const fetchCategories = async () => {
    ...
    try {
      // Fetch collaborator_id from subscriptions
      const response1 = await axios.get(
        `${import.meta.env.VITE_TESTING_API}/users/${user.id}/subscriptions?ecom_type=sofiaco`,
        {
          headers: {
            Authorization: `Bearer ${token}`, // Include token in the headers
          },
    ...
    if (categoryIds.length === 0) {
      ...
      const url = `${import.meta.env.VITE_TESTING_API}/categories?${queryParams}`;
