...
const AddBlog = () => {
  ...
  const HandleSubmit = async (statusId = 10) => {
    const formData = {
      title,
      introduction,
      description: data,
    ...
    try {
      await axios.post(`${import.meta.env.VITE_TESTING_API}/blogs`, formData);
      toast.success("Blog uploaded successfully", { position: "top-right", autoClose: 1500, hideProgressBar: true, closeOnClick: true, theme: "colored" });
      navigate('/main/blogs');
    } catch (error) {
    ...
    <div className={classes.maincontainer}>
      <div className={classes.itemsHeading}>
        <div className={classes.headContainer}>
          <p className={classes.itemHead}>{language === 'eng' ? "Write A Blog" : "Rédiger un blog"}</p>
        </div>
      </div>
      <Form layout="vertical" name="nest-messages" form={form} className={classes.card1}>
        ...
        <Form.Item name="description" style={{ width: '100%' }} label={<p style={{ color: 'var(--primary-color)', fontWeight: '500', fontFamily: 'var(--font-family)', margin: '0' }}>{language === 'eng' ? "Content" : "Contenu"}</p>}>
          <Input.TextArea name="description" rows={10} placeholder={language === 'eng' ? "Write your blog content here..." : "Rédigez le contenu de votre blog ici..."} style={{ border: '1px solid #D9D9D9', backgroundColor: "#F2F2F2", color: 'var(--primary-color)' }} onChange={(e) => setData(e.target.value)} />
        </Form.Item>
