...
const Details = () => {
  ...
  const handleSuivreCollab = async () => {
    ...
    try {
        ...
        const collaboratorsResponse = await axios.get(`${import.meta.env.VITE_TESTING_API_IMAGE}/collaborators?ecom_type=sofiaco`);
...
const handleSuivreCollection = async () => {
  ...
  try {
      // Fetch the list of collaborators
      const collectionsResponse = await axios.get(`${import.meta.env.VITE_TESTING_API_IMAGE}/collections?ecom_type=sofiaco`);
